...
**⚠️ NOTE:** This documentation refers mainly to  `Engine` to describe the architecture and behaviour. `AsycEngine` replicates the logic of `Engine` just offering asyc api. Whenever there is a reference to the async engine, a relevant difference in the beaviour and/or in the implications of using an async logic, the ⚠️ sybmol is used. Furthermore,  [Asyc VS Sync performance assesment](./asyncvssync.md) document has been added to report the performance assesment of using an async engine vs a sync one. 
