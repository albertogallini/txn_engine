...
- The solution scales horizontally as follows
  ...
  - 200 x 30 days  = 6.000 transaction per client per period have to be stored
  - 1B / 6.000 =  166.666 = ~166.000 (for simplicity) clients per machine.
  - We can add a node increasing throughput and without sacrificing latency, e.g.: a farm with 100 64Gb node can manage 16.6 M clients.

- The solution scales verticlly as follows
  - increase the node memory will allow to manage more clients per node. e.g.: 256 Gb memeory will allow to manage 66 M clients (which is in line with the # of customers of biggest retail banks on earth)

- Further optimiztions:
  - implement logic to periodically store the Engine state on disk or DB and keep in memory only the most active users e.g. by implementing lazy write caching logic. This will reduce the 
